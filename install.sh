...
T_en=(
    ...
    [sum_front_note]="(shared by every shop of this server; this shop: %s)"
    [sum_data]="Data"
...
T_fr=(
    ...
    [sum_front_note]="(partagé par toutes les boutiques de ce serveur ; cette boutique : %s)"
    [sum_data]="Données"
...
T_de=(
    ...
    [sum_front_note]="(von allen Shops dieses Servers geteilt; dieser Shop: %s)"
    [sum_data]="Daten"
...
T_es=(
    ...
    [sum_front_note]="(compartido por todas las tiendas de este servidor; esta tienda: %s)"
    [sum_data]="Datos"
...
T_it=(
    ...
    [sum_front]="Front HTTPS"
    [sum_front_note]="(condiviso da tutti i negozi di questo server; questo negozio: %s)"
    [sum_data]="Dati"
...
SUMMARY=$(
    ...
    L "$(tp sum_front)" "$PROXY   $(tp sum_front_note "sites/$SHOP.caddy")"
...
hr
printf '%s\n' "${TEAL}${B}$(tp installed_banner)${R}"
hr
# On screen: only the values to keep are green, the commands are yellow, everything else stays plain.
LS() { local bytes; bytes=$(printf '%s' "$1" | wc -c | tr -d ' '); printf "%-$(( W + bytes - $(clen "$1") ))s: ${GREEN}%s${R}%s\n" "$1" "$2" "${3:+   $3}"; }   # LS <label> <green value> [note]
LC() { local bytes; bytes=$(printf '%s' "$1" | wc -c | tr -d ' '); printf "${YELLOW}%-$(( W + bytes - $(clen "$1") ))s${R}: %s\n" "$1" "$2"; }               # LC <yellow command> <description>
printf '%s\n\n' "${GREEN}$(tp sum_title "$VERSION" "$SHOP" "$(date '+%Y-%m-%d %H:%M')")${R}"
LS "$(tp sum_addr)" "$URL"
LS "$(tp sum_finish)" "$URL/install.php" "$(tp sum_finish_note)"
LS "$(tp sum_admin)" "$URL/admin/login.php"
printf '\n%s\n' "$(tp sum_db)"
L "  $(tp sum_hostport)" "db / 3306   $(tp sum_prefilled)"
LS "  $(tp sum_name)" "$DB_NAME"
LS "  $(tp sum_user)" "$DB_USER"
LS "  $(tp sum_pass)" "$DB_PASSWORD"
LS "  $(tp sum_root)" "$DB_ROOT_PASSWORD"
printf '\n'
LS "$(tp sum_files)" "$SHOP_DIR" "$(tp sum_files_note)"
LS "$(tp sum_front)" "$PROXY" "$(tp sum_front_note "${GREEN}sites/$SHOP.caddy${R}")"
L "$(tp sum_data)" "$(tp sum_data_note "sellarus-$SHOP" "sellarus-$SHOP")"
LS "$(tp sum_others)" "$OTHERS_TXT" "$(tp sum_others_note)"
printf '\n%s\n' "$(tp sum_cmds)"
LC "  sellarus list" "$(tp cmd_list)"
LC "  sellarus status $SHOP" "$(tp cmd_status)"
LC "  sellarus update $SHOP" "$(tp cmd_update)"
LC "  sellarus backup $SHOP" "$(tp cmd_backup "$SHOP_DIR/backups/")"
LC "  sellarus logs $SHOP web" "$(tp cmd_logs)"
LC "  sellarus domain $SHOP <d>" "$(tp cmd_domain)"
LC "  sellarus restart $SHOP" "$(tp cmd_restart)"
hr
why "$(tp keep_note)"
